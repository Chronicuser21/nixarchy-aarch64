# Build a whole-disk image the Asahi installer can lay down: GPT with an ESP
# and a root partition, bootable via m1n1 + U-Boot on Apple Silicon.
#
# Ported from quinneden/nixos-asahi-package (MIT) -- the shape that installs
# NixOS on bare-metal Apple Silicon through the stock Asahi installer. The
# image is built in two qemu stages so the bootloader can be written while the
# ESP is a real block device; the partition-image trick in package.nix then
# turns the single .raw into the esp/ + root.img pair the Asahi installer
# expects in its os package.
{
  config,
  copyConfig ? null,
  diskSize ? "auto",
  ...
  lib,
  memSize ? 4096,
  name ? "omarchy-asahi-${version}-${fsType}-image",
  pkgs,
